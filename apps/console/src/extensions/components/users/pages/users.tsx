...
const UsersPage: FunctionComponent<UsersPageInterface> = (
    ...
    const addUserOptions = [
        { key: 1, text: "Consumer", value: "Consumer", "data-testid": "users-add-user-dropdown-consumer" },
        { key: 2, text: "Guest", value: "Guest", "data-testid": "users-add-user-dropdown-guest" }
    ];

    const handleDropdownItemChange = (value: string): void => {
        if (value === "Consumer") {
            setSelectedAddUserType(UserAccountTypes.CONSUMER);
            handleAddNewUserWizardClick()
        } else if (value === "Guest") {
            setSelectedAddUserType(UserAccountTypes.GUEST);
            handleAddNewUserWizardClick()
        }
    };
