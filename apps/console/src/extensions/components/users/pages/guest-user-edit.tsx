...
import { hasRequiredScopes, resolveUserDisplayName, resolveUserEmails, isFeatureEnabled } from "@wso2is/core/helpers";
import {
    AlertInterface,
    AlertLevels,
            ...
            description={
                <div className="with-label">
                    <div className="with-description">
                        { t("" +user.emails && user.emails !== undefined ? resolvePrimaryEmail(user?.emails) :
                            user.userName)}
                    </div>
                    <Label>
                        { t("extensions:manage.users.editUserProfile.userId") }: { user.id }
                    </Label>
                </div>
            }
            image={ (
