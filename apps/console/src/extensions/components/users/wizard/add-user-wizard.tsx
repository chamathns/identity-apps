    ...
    const showSteps = (): boolean => {
        if (!conditionallyShowStepper) {
            ...
            if (fixedGroupList?.length) {
                return showStepper && ((defaultUserTypeSelection !== UserAccountTypes.CONSUMER ||
        ...
        wizardSteps && isStepsUpdated ? (
            ...
            >
                <Modal.Header className="wizard-header">
                    { defaultUserTypeSelection === UserAccountTypes.CONSUMER && "Add Consumer User" }
                    { defaultUserTypeSelection === UserAccountTypes.GUEST && "Invite Guest User" }
                    {
                    ...
                    <Heading as="h6">
                        { defaultUserTypeSelection === UserAccountTypes.CONSUMER && "Follow the steps to add a new user" }
                    </Heading>
                </Modal.Header>
                { showSteps() &&
                     (
                        <Modal.Content className="steps-container">
                            {  defaultUserTypeSelection === UserAccountTypes.GUEST ?
