...
const GuestUsersPage: FunctionComponent<GuestUsersPageInterface> = (
    ...
    useEffect(() => {
        if (!onboardedGuestUserList || onboardedGuestUserList.length==0) {
            if (guestUsersList.filter((invitation) => invitation.status === InvitationStatuses.PENDING.toUpperCase()).length>0) {
                handleInvitationStatusOptionChange(InvitationStatuses.PENDING);
            } else if (guestUsersList.filter((invitation) => invitation.status === InvitationStatuses.EXPIRED.toUpperCase()).length>0) {
