...
import React, { FunctionComponent, ReactElement, Suspense, useState, useEffect } from "react";
    ...
    useEffect(() => {
        ...
        if (connectorProperties && Array.isArray(connectorProperties) && connectorProperties?.length > 0) {
            for (const property of connectorProperties) {
                if (property.name === ServerConfigurationsConstants.ADMIN_FORCED_PASSWORD_RESET_EXPIRY_TIME) {
